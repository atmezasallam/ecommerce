...
/**
 * Return the length of the plain text inside an HTML string.
 * Used by Zod schemas to validate rich-text fields.
 */
export function getPlainTextLength(html: string): number {
  // Browser path (rich-text editor often runs in the browser).
  ...
  // Server/worker path: best-effort stripping.
  const text = html
    .replace(/<[^>]*>/g, " ")
    .replace(/&nbsp;/gi, " ")
    .replace(/&amp;/gi, "&")
