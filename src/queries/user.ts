...
export const followStore = async (storeId: string): Promise<boolean> => {
  try {
    const userData = await ensureDbUserForClerk();
...
export const placeOrder = async (
  ...
  // Fetch product, variant, and size data from the database for validation
  const validatedCartItems = await Promise.all(
    cartItems.map(async (cartProduct: CartItem) => {
      const { productId, variantId, sizeId, quantity } = cartProduct;
  ...
  const grandCartTotal = validatedCartItems.reduce((sum, item) => sum + item.totalPrice, 0);
  const now = new Date();
  const couponDatesOk =
    cartCoupon != null &&
    now > new Date(cartCoupon.startDate) &&
    now < new Date(cartCoupon.endDate);

  let globalDiscountPool = 0;
  const useGlobalCoupon = Boolean(
    couponDatesOk && cartCoupon?.isGlobal && grandCartTotal > 0
  );
  if (useGlobalCoupon && cartCoupon) {
    globalDiscountPool = (grandCartTotal * cartCoupon.discount) / 100;
  }
  ...
  for (const [storeId, items] of Object.entries(groupedItems)) {
    ...
    // Calculate discount: platform-wide coupon splits by each group's share of cart total
    let discountedAmount = 0;
    if (couponDatesOk && cartCoupon) {
      if (useGlobalCoupon) {
        discountedAmount =
          grandCartTotal > 0
            ? (groupedTotalPrice / grandCartTotal) * globalDiscountPool
            : 0;
      } else if (cartCoupon.storeId && storeId === cartCoupon.storeId) {
    ...
    const orderGroup = await db.orderGroup.create({
      data: {
        ...
        couponId: discountedAmount > 0 && cartCoupon ? cartCoupon.id : null,
      },
    });
    ...
    if (currentDate > startDate && currentDate < endDate) {
      if (coupon.isGlobal) {
        ...
        if (applicableStoreItems.length > 0) {
          ...
          total -= (storeSubTotal * coupon.discount) / 100;
        }
      }
    }
  }
