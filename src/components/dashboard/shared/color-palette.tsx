...
const ColorPalette: FC<ColorPaletteProps> = ({
  ...
  const Color = ({ color }: { color: string }) => {
    return (
      ...
      >
        {/* Color label */}
        <div className="w-full h-8 text-center text-xs font-semibold absolute -top-6 text-primary">
          {color}
        </div>
      </div>
    );
  ...
  return (
    <div className="pt-10 w-[320px] h-[160px] rounded-b-md overflow-hidden">
      ...
      <div className="w-[320px] h-[180px] rounded-md perspective-1000">
        ...
        <div className="relative w-full flex items-center justify-center bg-surface h-16 rounded-t-md">
          {/* Active color circle */}
          <div
            className="absolute w-16 h-16 grid place-items-center shadow-lg rounded-full -top-10"
            style={{ backgroundColor: activeColor || "var(--site-surface)" }}
          >
            {/* Spinner icon */}
            <svg
