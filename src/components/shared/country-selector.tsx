...
import { SelectMenuOption } from "@/src/lib/types";
        ...
        <AnimatePresence>
          {open && (
            <motion.ul
              ...
              className="absolute z-20 mt-1 shadow-lg w-full bg-white max-h-80 rounded-md text-base ring-1 ring-black/10 focus:outline-none sm:text-sm"
              tabIndex={-1}
              role="listbox"
              aria-labelledby="listbox-label"
            ...
            >
              <div className="sticky top-0 z-10 bg-white">
                <li className=" text-subtle cursor-default select-none relative py-2 px-3">
                  <input
                    type="search"
                    name="search"
                    autoComplete={"off"}
              ...
              >
                {COUNTRIES.filter((country) =>
                  ...
                  <li className="text-subtle cursor-default select-none relative py-2 pl-3 pr-9">
                    No countries found
                  </li>
                ) : (
                  COUNTRIES.filter((country) =>
                    ...
                    return (
                      <li
                        key={`${id}-${index}`}
                        className="text-subtle cursor-default select-none relative py-2 pl-3 pr-9 flex items-center hover:bg-base transition"
                        id="listbox-option-0"
                        role="option"
                        onClick={() => {
                          onChange(value.name);
                      ...
                      >
                        ...
                        {value.name === selectedValue.name ? (
                          <span className="text-[#95CFB2] absolute inset-y-0 right-0 flex items-center pr-8">
                            <svg
                              className="h-5 w-5"
                              xmlns="http://www.w3.org/2000/svg"
                              viewBox="0 0 20 20"
